...
- 隐私化：小说全文不上传、不分享、不转发，只在浏览器本地读取和处理；反馈时只发送用户主动提交的当前词和原句短片段。

## 功能

- 本地选择 `.txt` 或可复制文字的 `.pdf` 小说文件，通过浏览器读取；PDF 使用 PDF.js 提取文字层。
- 小说全文不上传服务器；用户主动提交反馈时，当前词和原句短片段会交给 AI 审核。
- 使用本地 `src/data/cet4-map.json` 做中英词表映射；同一中文词会保留多个英文候选。
- 多候选词按人工优先级和词表顺序稳定替换，只有明确语境提示或用户已保存的同句纠正才会切换；无法安全压缩成单个英文词的高风险词会保留中文。
- 默认使用沉浸模式，约 `35%` 候选词替换，并按段落均匀分布。
- 点击英文单词显示中文释义、IPA 音标、原中文词和原句；当前词表 3792 条记录均有音标。
- 使用 `IndexedDB + Dexie.js` 保存阅读进度、生词本、替换记录、黑名单、翻译反馈和题目记录。
- 阅读章节后生成最多 5 道基于原句的填空题。
- 支持 PWA 安装与离线应用壳缓存。
- TXT 支持 UTF-8、UTF-16 和 GB18030/GBK 小说编码；扫描版图片 PDF 需要先 OCR。
- 单词详情把“反馈翻译”和“加入黑名单”分开；反馈弹窗可选择释义、词性、断词或语境问题，并填写“我建议改成”。
- 反馈会先保存在本地，再尝试通过 Cloudflare Pages Function 交给 AI 审核；反馈不会自动改词表，也不会自动加入黑名单。
...
- 反馈审核接口：通过 Cloudflare Pages Function 服务端调用 AI，API Key 不放进浏览器。

## 隐私说明

当前 MVP 不包含任何上传小说全文的逻辑，也没有后端登录、云同步或统计分析。小说文本只存在于当前浏览器运行时内存中；只有用户主动反馈时，反馈中的当前词和原句短片段会发送给 AI 审核接口。阅读进度、生词、替换记录、黑名单、翻译反馈和练习记录仍写入本地 IndexedDB。

### 启用 AI 反馈

在 Cloudflare Pages 项目的生产环境变量中添加 `OPENAI_API_KEY`；可选添加 `OPENAI_MODEL` 指定模型，默认使用 `gpt-5.4-mini`。未配置时，反馈仍会保存在用户本地，但页面会提示稍后提交。
