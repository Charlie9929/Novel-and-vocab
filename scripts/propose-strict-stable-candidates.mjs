...
function addQuotedCandidateIds(source, declarationPattern = null) {
  ...
  for (const chunk of chunks) {
    for (const match of chunk.matchAll(/"([^"\n]+:[^"\n]+:(?:noun|verb|adjective|adverb))"/g)) {
      curatedCandidateIds.add(match[1]);
    }
  }
}
...
proposals.sort((left, right) => right.developmentSupport - left.developmentSupport
...
console.log(JSON.stringify({ vocabularyId, proposed: proposals.length, output: outputPath }));
