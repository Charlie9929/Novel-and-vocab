...
const outputPath = resolve(args.get("--out") ?? `tests/private-input/quality/annotation-${split ?? "all"}.json`);
...
const pool = manifest.samples.filter((sample) => sample.split === split && sample.annotationStatus === "unreviewed");
...
for (const sample of selected) {
  ...
  packet.push({
    ...
    instruction: "Label replace only if the exact span and one candidate are clearly correct in this context; otherwise keepChinese. Do not infer from the suggested candidate order.",
  });
}
await mkdir(dirname(outputPath), { recursive: true });
await writeFile(outputPath, `${JSON.stringify({ schemaVersion: 1, split, packet }, null, 2)}\n`);
console.log(JSON.stringify({ split, samples: packet.length, output: outputPath }));
